...
public:
	...
	void RobotInit() {
		...
		flmotor->ConfigSelectedFeedbackSensor(CTRE_MagEncoder_Absolute, 0, 10);
		frmotor->ConfigSelectedFeedbackSensor(CTRE_MagEncoder_Absolute, 0, 10);

		frmotor->ConfigPeakOutputForward(0.5, 10); //configure back to max before teleop
		frmotor->ConfigPeakOutputReverse(-0.5, 10);
	...
	void AutonomousPeriodic() {
		if(lEncoderCount>=target && rEncoderCount>=target && !distReached){
